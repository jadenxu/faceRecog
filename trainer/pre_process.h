...
#ifndef NUM
#define NUM
const int NUM_F = 4000;
const int NUM_NF = 8000;
int NUM_T = 32384;
#endif
  ...
  vector<vector<int> >& nf_val)
{
	...
	for(int n = h_n[1]; n < h_n[2]; n++)
	{
		...
		{
			i = ftr[n][0], j = ftr[n][1], p = ftr[n][2], q = ftr[n][3];
			f_val[n][m] = cal_sq(i,j,p,p*2/3+q/3, i_faces[m]) + cal_sq(i,j,p/3+q*2/3,q, i_faces[m]) - cal_sq(i,j,p*2/3+q/3,p/3+q*2/3, i_faces[m]);
		}
		for(int m = 0; m < NUM_NF; m++)
		{
			i = ftr[n][0], j = ftr[n][1], p = ftr[n][2], q = ftr[n][3];
			nf_val[n][m] = cal_sq(i,j,p,p*2/3+q/3, i_nfaces[m]) + cal_sq(i,j,p/3+q*2/3,q, i_nfaces[m]) - cal_sq(i,j,p*2/3+q/3,p/3+q*2/3, i_nfaces[m]);
		}
	}
	...
	for(int n = h_n[2]; n < h_n[3]; n++)
	{
		...
		{
			i = ftr[n][0], j = ftr[n][1], p = ftr[n][2], q = ftr[n][3];
			f_val[n][m] = cal_sq(i,i*2/3+j/3,p,q,i_faces[m]) + cal_sq(i/3+j*2/3,j,p,q,i_faces[m]) - cal_sq(i*2/3+j/3,i/3+j*2/3,p,q,i_faces[m]);
		}
		for(int m = 0; m < NUM_NF; m++)
		{
			i = ftr[n][0], j = ftr[n][1], p = ftr[n][2], q = ftr[n][3];
			nf_val[n][m] = cal_sq(i,i*2/3+j/3,p,q,i_nfaces[m]) + cal_sq(i/3+j*2/3,j,p,q,i_nfaces[m]) - cal_sq(i*2/3+j/3,i/3+j*2/3,p,q,i_nfaces[m]);
		}
	}
